...
function indekseGoreFenomen(fenomenler,index) {
  ...
  let text = `${index}. indexte bulunan fenomen: ${x}`
  return text
}
...
function fenomenEkle(fenomenler,gNumber,gProfile,gFollowers,gPosts,gPlatform) {
  let yeniObje = {};
  yeniObje.number = gNumber;
  yeniObje.profile = gProfile;
  yeniObje.followers = gFollowers;
  yeniObje.posts = gPosts;
  yeniObje.platform = gPlatform;
  let yeniFenomenler = [...fenomenler];
  yeniFenomenler.push(yeniObje);
  return yeniFenomenler;
}

console.log(fenomenEkle(fenomenler,6,"probiss",80000000,2022,"9gag"));
...
function fenomenGonderimSayisi(dizi,profileName){
  let fenomenPostSayisi ;
  for(let i=0; i<dizi.length; i++){
    if( profileName === dizi[i].profile){
      fenomenPostSayisi = dizi[i].posts;
    }
}
...
function platformaGoreCokGonderiYapanFenomen(dizi,platform){
  ...
  for(let i=0; i<dizi.length; i++){
    if(dizi[i].platform===platform){
      if (dizi[i].posts !== "NA" && dizi[i].posts > max ){
        max = dizi[i].posts;
        fen = dizi[i].profile;
      }
    }
